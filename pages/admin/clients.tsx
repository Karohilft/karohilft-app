          ...
          : clients.map(c => (
            <div key={c.id} style={{ background: '#fff', borderRadius: 'var(--r-md)', padding: '14px 18px', marginBottom: 10, boxShadow: 'var(--shadow-sm)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                {c.notes && <div style={{ fontSize: 13, color: 'var(--mid)', marginTop: 2, fontStyle: 'italic' }}>{c.notes}</div>}
              </div>
              <button onClick={() => del(c.id)} style={{ background: 'transparent', border: 'none', color: '#ccc', cursor: 'pointer', fontSize: 18, padding: '0 4px' }} title="Löschen">×</button>
            </div>
          ))}
      </div>
    </div>
