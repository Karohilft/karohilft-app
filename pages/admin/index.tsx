      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        <div style={{ background: '#fff', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-md)', padding: '32px 28px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {menu.map(item => (
              <a key={item.href} href={item.href} style={{ display: 'block', padding: '16px 20px', borderRadius: 'var(--r-md)', border: '1.5px solid var(--cream-alt)', background: 'var(--cream)', textDecoration: 'none' }}>
                <div style={{ fontWeight: 600, color: 'var(--dark)', marginBottom: 2 }}>{item.label}</div>
                <div style={{ fontSize: 14, color: 'var(--mid)' }}>{item.desc}</div>
              </a>
            ))}
          </div>
          <button onClick={() => getSupabase().auth.signOut().then(() => router.replace('/login'))} style={{ marginTop: 20, padding: '12px 24px', borderRadius: 'var(--r-pill)', border: '1.5px solid rgba(28,24,20,.12)', background: 'transparent', color: 'var(--mid)', fontSize: 15, cursor: 'pointer', width: '100%' }}>
            Abmelden
          </button>
        </div>
      </div>
