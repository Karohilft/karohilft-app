...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { to, klient, rechnungsnr, pdfBase64 } = req.body as {
    to: string; klient: string; rechnungsnr: string; pdfBase64: string
  }
...
im Anhang finden Sie Ihre Rechnung${rechnungsnr ? ` (${rechnungsnr})` : ''}${klient ? ` für die Betreuung von ${klient}` : ''}.
...
www.karohilft.at`
  ...
  const filename = `Rechnung${rechnungsnr ? '-' + rechnungsnr : ''}${klient ? '-' + klient.replace(/\s+/g, '-') : ''}.pdf`

  const { error } = await resend.emails.send({
    from: 'Karohilft <office@karohilft.at>',
    to,
    subject: `Rechnung${klient ? ' für ' + klient : ''}${rechnungsnr ? ' · ' + rechnungsnr : ''}`,
