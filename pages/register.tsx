...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true)
    setErr(null)
    const { error } = await getSupabase().auth.signUp({ email, password, options: { data: { full_name: name, role: 'user' } } })
    setBusy(false)
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--glass-bg)', borderRadius: 'var(--r-lg)', padding: '40px 32px', maxWidth: 420, width: '100%', textAlign: 'center', boxShadow: 'var(--shadow-md)' }}>
        ...
        <p>Wir haben dir eine Bestätigungsmail geschickt.</p>
      </div>
    </div>
  )
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ background: 'var(--glass-bg)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: '1px solid var(--glass-border)', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-md)', padding: '40px 32px 36px' }}>
          ...
          <form onSubmit={onSubmit} style={{ display: 'grid', gap: 14 }}>
            ...
            <input placeholder="Passwort" type="password" value={password} onChange={e => setPassword(e.target.value)} style={{ padding: '13px 16px', border: '1.5px solid rgba(28,24,20,.12)', borderRadius: 'var(--r-sm)', fontSize: 16, margin: 0 }} />
          ...
          <p style={{ marginTop: 16, fontSize: 14, color: 'var(--mid)', textAlign: 'center' }}>Bereits registriert? <a href="/login" style={{ color: 'var(--rose)' }}>Einloggen</a></p>
        </div>
      </div>
    </div>
  )
